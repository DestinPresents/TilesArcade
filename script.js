...
const state={
 ...
 maxUnlocked:numStore("tt_max_unlocked",numStore("tt_level",21,1,MAX_LEVEL),1,MAX_LEVEL),
...
function save(){state.maxUnlocked=Math.max(Number(state.maxUnlocked)||1,Number(state.level)||1);localStorage.setItem("tt_level",state.level);localStorage.setItem("tt_max_unlocked",state.maxUnlocked);localStorage.setItem("tt_coins",state.coins);localStorage.setItem("tt_streak",state.streak);localStorage.setItem("tt_daily",state.lastDaily);localStorage.setItem("tt_pu",JSON.stringify(state.pu));localStorage.setItem("tt_sound",state.sound?"1":"0");localStorage.setItem("tt_music",state.music?"1":"0");localStorage.setItem("tt_extra_slot",state.extraSlot?"1":"0");try{if(window.TileTrailsAuth&&typeof window.TileTrailsAuth.queueSave==="function")window.TileTrailsAuth.queueSave()}catch(_){}}
...
function buildMap(){const m=$("#mapPath");m.innerHTML="";const frag=document.createDocumentFragment();const unlocked=Math.max(Number(state.maxUnlocked)||1,Number(state.level)||1);for(let n=1;n<=MAX_LEVEL;n++){const b=document.createElement("button");const isUnlocked=n<=unlocked;b.className="map-node "+(n<state.level?"done ":"")+(n===state.level?"current ":"")+(!isUnlocked?"locked":"");b.textContent=!isUnlocked?"🔒 "+n:(n<state.level?"✓ "+n:n);if(isUnlocked)b.onclick=()=>startLevel(n);frag.appendChild(b)}m.appendChild(frag)}
...
function win(){
  ...
  if(completed<MAX_LEVEL){state.maxUnlocked=Math.max(Number(state.maxUnlocked)||1,next);state.level=next;}else{state.maxUnlocked=MAX_LEVEL;state.level=MAX_LEVEL;}
...
function buyExtraSlot(){
  ...
  state.coins-=20;state.extraSlot=true;save();updateCoins();updateShopCoins();renderTray();try{if(window.TileTrailsAuth&&typeof window.TileTrailsAuth.saveNow==="function")window.TileTrailsAuth.saveNow().catch(function(e){console.warn("Extra-slot cloud save failed",e)})}catch(_){ }playSound('buy');vibrate([16,25]);showToast('EXTRA SLOT UNLOCKED • 20 COINS','buy');
}
function buyPowerup(type,price){
  if(state.coins<price){playSound('lose');vibrate([50]);showToast('NOT ENOUGH COINS','bad');return;}
  state.coins-=price;state.pu[type]++;save();updateCoins();updateShopCoins();requestAnimationFrame(updateShopCoins);updatePowerups();try{if(window.TileTrailsAuth&&typeof window.TileTrailsAuth.saveNow==="function")window.TileTrailsAuth.saveNow().catch(function(e){console.warn("Power-up cloud save failed",e)})}catch(_){ }playSound('buy');vibrate([16,25]);showToast(`${type.toUpperCase()} +1`,'buy');
}
function updateShopCoins(){const value=state.coins.toLocaleString();const el=$("shopCoinCount");if(el)el.textContent=value;document.querySelectorAll(".wallet strong").forEach(function(w){var coin=w.querySelector("#shopCoinCount");if(coin)coin.textContent=value;});} function openShop(){modal(`<div class="shop-hero"><div class="shop-icon">🛒</div><div><div class="eyebrow">TILE TRAILS</div><h2>SHOP</h2><p>Use coins earned by playing.</p></div></div><div class="wallet"><span>YOUR COINS</span><strong>🪙 <span id="shopCoinCount">${state.coins.toLocaleString()}</span></strong></div><div class="shop-title">EXTRA TRAY SLOT</div><div class="extra-slot-card ${state.extraSlot?'unlocked':''}"><div class="extra-slot-icon">＋</div><div><strong>${state.extraSlot?'Extra Slot Unlocked':'7th Slot'}</strong><small>${state.extraSlot?'You can now use all 7 tray spaces.':'Unlock the 7th tray space for 20 coins.'}</small></div>${state.extraSlot?'<span class="owned-badge">✓ OWNED</span>':'<button onclick="buyExtraSlot();openShop()">🪙 20</button>'}</div><div class="shop-title">POWER-UPS</div><div class="shop-cards"><div class="shop-card"><div class="shop-card-icon">↩</div><div><strong>Undo</strong><small>Restore your last move</small></div><button onclick="buyPowerup('undo',50)">🪙 50</button></div><div class="shop-card"><div class="shop-card-icon">🔀</div><div><strong>Shuffle</strong><small>Rearrange the board</small></div><button onclick="buyPowerup('shuffle',100)">🪙 100</button></div><div class="shop-card"><div class="shop-card-icon">🎯</div><div><strong>Auto Shot</strong><small>Automatically clears one matching triple</small></div><button onclick="buyPowerup('hint',50)">🪙 50</button></div></div><div class="shop-title">REDEEM CODE</div><div class="redeem-box"><input id="redeemInput" maxlength="20" placeholder="Enter code"><button onclick="redeemCode()">REDEEM</button></div><div class="redeem-note">Each code can be redeemed once on this device.</div><button class="secondary-action full" onclick="closeModal()">CLOSE</button>`)}
...
function openPrivacy(){modal(`<div class="settings-head"><div class="settings-icon">🔒</div><div><div class="eyebrow">TILE TRAILS</div><h2>PRIVACY POLICY</h2></div></div><div class="privacy-copy" style="max-height:58vh;overflow:auto;text-align:left;padding-right:4px;font-size:11px;line-height:1.6;color:#5f7d89"><p><b>Last updated: 11 September 2026</b></p><p>Tile Trails ("we", "us", or "our") respects your privacy. This Privacy Policy explains what information may be collected when you use the Tile Trails game, why it is used, how it is stored, and the choices available to you.</p><h3>1. Information We Collect</h3><p><b>Account information:</b> If you create an account with email/password or continue with Google, we may process your email address, Firebase account identifier, display/player name, and authentication information required to sign you in securely.</p><p><b>Game information:</b> We may store your Player ID, level progress, maximum unlocked level, coins, streak/daily reward information, power-ups, settings, and other game-progress data so your progress can be restored when you sign in.</p><p><b>Local device data:</b> Tile Trails uses browser storage such as localStorage to remember certain game settings and cached game progress on your device.</p><p>We do not intentionally collect precise location, contacts, photos, microphone recordings, or payment-card information through the game.</p><h3>2. How We Use Information</h3><p>We use the information described above to provide account login, email verification, Google sign-in, password recovery, cloud save/synchronization, Player ID display, game progression, rewards, settings, and basic security/anti-abuse functions.</p><h3>3. Firebase and Google Services</h3><p>Authentication and cloud game data are provided through Google Firebase services. If you choose Google sign-in, Google processes the information necessary for authentication under Google's own policies. Firebase may process authentication and database information on our behalf. These services are used to operate the account and cloud-save features of Tile Trails.</p><h3>4. Passwords and Security</h3><p>Tile Trails does not receive or store your plain-text password in its game database. Password authentication is handled by Firebase Authentication. Email/password accounts may be required to verify their email address before access is allowed. Password-reset requests are handled through Firebase's authentication email system.</p><h3>5. Cookies and Browser Storage</h3><p>Tile Trails may use browser storage technologies to remember game state, preferences, and temporary account-related information. These values help the game work correctly and are not intended to be used for advertising profiles.</p><h3>6. Sharing of Information</h3><p>We do not sell or rent your personal information. Information may be processed by service providers such as Firebase/Google when necessary to provide authentication, cloud storage, security, and related game functionality. We may also disclose information when required by applicable law or when reasonably necessary to protect the game, users, or our rights.</p><h3>7. Data Retention</h3><p>Account and cloud-save information may remain associated with your Firebase account while the account is active so that your progress can be restored. Local browser data can remain on your device until it is cleared by the browser or by the user.</p><h3>8. Your Choices</h3><p>You may choose whether to create an account or use available guest functionality. You can sign out of your account at any time. You may clear browser storage through your browser settings. If you want an account or associated game data reviewed or deleted, use the official Tile Trails support/contact channel available to you and provide only the information needed to identify the account.</p><h3>9. Children's Privacy</h3><p>Tile Trails is not intended to knowingly collect personal information from children in violation of applicable privacy laws. If a parent or guardian believes a child has provided personal information inappropriately, they should contact the developer through the official support/contact channel so the information can be reviewed and, where appropriate, removed.</p><h3>10. Third-Party Links and Services</h3><p>The game may use or connect to third-party services such as Google/Firebase. Their handling of information is governed by their own privacy policies and terms. Tile Trails is not responsible for the privacy practices of independent third-party services.</p><h3>11. Changes to This Policy</h3><p>We may update this Privacy Policy when the game, services, or applicable requirements change. The updated version will be made available through the game, with the "Last updated" date changed accordingly.</p><h3>12. Contact</h3><p>For privacy questions or requests relating to Tile Trails, please use the official developer/support contact channel associated with the game. Please do not send passwords, verification codes, or other sensitive authentication secrets.</p><p><b>By using Tile Trails, you acknowledge that you have read this Privacy Policy and understand how information is handled as described above.</b></p></div><button class="secondary-action full" onclick="closeModal()">CLOSE</button>`)}
// Firebase bridge: keeps the live game state and Firestore state synchronized.
window.TileTrailsGame={
  getState:function(){
    return {
      level:Number(state.level)||1,
      maxUnlocked:Math.max(Number(state.maxUnlocked)||1,Number(state.level)||1),
      coins:Math.max(0,Number(state.coins)||0),
      streak:Math.max(0,Number(state.streak)||0),
      lastDaily:state.lastDaily||"",
      pu:{undo:Number(state.pu.undo)||0,shuffle:Number(state.pu.shuffle)||0,hint:Number(state.pu.hint)||0},
      sound:!!state.sound,
      music:!!state.music,
      extraSlot:!!state.extraSlot
    };
  },
  applyCloudState:function(data){
    if(!data)return;
    if(data.level!=null)state.level=Math.min(MAX_LEVEL,Math.max(1,Number(data.level)||1));
    if(data.maxUnlocked!=null)state.maxUnlocked=Math.min(MAX_LEVEL,Math.max(1,Number(data.maxUnlocked)||1));
    state.maxUnlocked=Math.max(state.maxUnlocked,state.level);
    if(data.coins!=null)state.coins=Math.max(0,Number(data.coins)||0);
    if(data.streak!=null)state.streak=Math.max(0,Number(data.streak)||0);
    if(data.lastDaily!=null)state.lastDaily=String(data.lastDaily||"");
    if(data.pu&&typeof data.pu==='object'){
      state.pu={undo:Math.max(0,Number(data.pu.undo)||0),shuffle:Math.max(0,Number(data.pu.shuffle)||0),hint:Math.max(0,Number(data.pu.hint)||0)};
    }
    if(data.sound!=null)state.sound=!!data.sound;
    if(data.music!=null)state.music=!!data.music;
    if(data.extraSlot!=null)state.extraSlot=!!data.extraSlot;
    localStorage.setItem("tt_level",state.level);
    localStorage.setItem("tt_max_unlocked",state.maxUnlocked);
    localStorage.setItem("tt_coins",state.coins);
    localStorage.setItem("tt_streak",state.streak);
    localStorage.setItem("tt_daily",state.lastDaily);
    localStorage.setItem("tt_pu",JSON.stringify(state.pu));
    localStorage.setItem("tt_sound",state.sound?"1":"0");
    localStorage.setItem("tt_music",state.music?"1":"0");
    localStorage.setItem("tt_extra_slot",state.extraSlot?"1":"0");
    updateCoins();
    updatePowerups();
    buildPreview();
    buildMap();
