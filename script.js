...
function buyExtraSlot(){
  ...
  state.coins-=20;state.extraSlot=true;save();updateCoins();renderTray();try{if(window.TileTrailsAuth&&typeof window.TileTrailsAuth.saveNow==="function")window.TileTrailsAuth.saveNow().catch(function(e){console.warn("Extra-slot cloud save failed",e)})}catch(_){ }playSound('buy');vibrate([16,25]);showToast('EXTRA SLOT UNLOCKED • 20 COINS','buy');
}
function buyPowerup(type,price){
  if(state.coins<price){playSound('lose');vibrate([50]);showToast('NOT ENOUGH COINS','bad');return;}
  state.coins-=price;state.pu[type]++;save();updateCoins();updatePowerups();try{if(window.TileTrailsAuth&&typeof window.TileTrailsAuth.saveNow==="function")window.TileTrailsAuth.saveNow().catch(function(e){console.warn("Power-up cloud save failed",e)})}catch(_){ }playSound('buy');vibrate([16,25]);showToast(`${type.toUpperCase()} +1`,'buy');
}
